...
description: Practical guidelines for effective work with the AI Advisor — formulating questions, attaching files, handling citations, and recognizing budget exhaustion signals.
---
...
When a query exceeds the single-request budget — usually a multi-source question with a very broad scope — the response concludes with a note regarding the incomplete portion:
...
This is a valid result. The recommended action is to accept the partial response and submit a narrower follow-up query. If budget exhaustion occurs regularly for a given question type, the question is likely too broad — two narrower questions yield a better combined result than one very broad one.
